...
RSpec.describe Lesson, type: :model do
  ...
  describe "validations" do
    ...
    context "開始時間より終了時間のほうが若い時" do
      before do
        lesson.start_time = Time.zone.now
        lesson.end_time = Time.zone.now - 1.hour
      end
    ...
    context "開始時間と終了時間が異なる日の時" do
      before do
        lesson.start_time = Time.zone.now
        lesson.end_time = Time.zone.now + 1.day
      end
    ...
    context "既に登録済みのレッスンがある時" do
      ...
      let!(:lesson_1) { create(:lesson, start_time: time - 1.hour, end_time: time, teacher: teacher) }
      let!(:lesson_2) { create(:lesson, start_time: time + 1.hour, end_time: time + 2.hour, teacher: teacher) }
