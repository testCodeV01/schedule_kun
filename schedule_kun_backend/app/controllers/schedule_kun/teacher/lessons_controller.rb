class ScheduleKun::Teacher::LessonsController < ScheduleKun::Teacher::ApplicationController
  ...
  # GET /schedule_kun/teacher/day_schedule
  def index
  ...
  # GET /schedule_kun/teacher/day_schedule/new
  def new
    ...
    json[:branches] = branches.each_with_object([]) do |branch, arr|
      arr << branch.client_attributes.merge!(lesson_rooms: branch.lesson_rooms)
    end
  ...
  # POSt /schedule_kun/teacher/day_schedule
  def create
    @lesson = current_teacher.lessons.build(day_schedule_params)
    @lesson.save!
